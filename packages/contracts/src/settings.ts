...
export const HermesSettings = makeProviderSettingsSchema(
  {
    ...
    profile: Schema.String.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Profile",
        description:
          'Hermes profile name. Empty or "default" uses ~/.hermes; named profiles use ~/.hermes/profiles/<name>.',
        providerSettingsForm: { placeholder: "default", clearWhenEmpty: "omit" },
      }),
    ),
    authMethodId: Schema.String.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Auth method ID",
        description:
          "Optional override for the Hermes ACP auth method (e.g. openrouter). Leave empty to auto-detect from Hermes config.",
        providerSettingsForm: { placeholder: "openrouter", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
  ...
  {
    order: ["binaryPath", "profile", "authMethodId"],
  },
);
