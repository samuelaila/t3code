...
describe("sortThreadsForSidebar", () => {
  ...
  it("does not reshuffle working rows when turn/update timestamps advance", () => {
    ...
    const sortedOnce = sortThreadsForSidebar(base);
    const sortedAgain = sortThreadsForSidebar(
      base.map((thread) =>
  ...
  it("drops a row out of the Done band once lastVisited clears the pill", () => {
    ...
    expect(sortThreadsForSidebar([working, finished]).map((t) => t.id)).toEqual([
      "was-done",
      "working-newer",
    ]);
    ...
    expect(sortThreadsForSidebar([working, visited]).map((t) => t.id)).toEqual([
      "working-newer",
      "was-done",
    ]);
  });
