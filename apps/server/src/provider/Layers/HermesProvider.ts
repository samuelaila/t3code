...
const runHermesVersionCommand = (
  ...
  Effect.gen(function* () {
    ...
    const spawnCommand = yield* resolveSpawnCommand(command, ["acp", "--check"], {
      env: environment,
    });
...
> {
  ...
  if (Option.isNone(versionResult.success)) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: "Hermes CLI is installed but timed out while running `hermes acp --check`.",
      },
    });
  }
  ...
  const authMethodId = yield* resolveHermesAuthMethodId(hermesSettings, environment);
  if (!authMethodId) {
    return buildServerProvider({
      presentation: HERMES_PRESENTATION,
      enabled: hermesSettings.enabled,
      ...
      probe: {
        ...
        auth: { status: "unknown" },
        message:
          "Hermes CLI is installed but no provider credentials are configured. Run `hermes model` or `hermes acp --setup`.",
      },
    });
  }
